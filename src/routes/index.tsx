...
function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white font-sans">
      {/* Top Bar */}
      <div className="bg-[#D4AF37] text-slate-900 py-2 text-center text-sm font-bold uppercase tracking-wider">
        ⚠️ OFERTA VÁLIDA SOMENTE HOJE - {date}
      </div>

      {/* Hero Section */}
      <section className="py-20 px-6 flex flex-col items-center text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="px-4 py-1 rounded-full bg-slate-800/50 border border-slate-700 text-sm mb-6 uppercase tracking-widest">
        ...
        <div className="w-full max-w-sm aspect-[9/16] bg-slate-800 rounded-3xl mb-12 shadow-[0_0_30px_rgba(212,175,55,0.2)]" />
        ...
        <motion.button 
          ...
          className="bg-[#D4AF37] hover:bg-[#FFD700] text-slate-900 font-black text-lg py-6 px-10 rounded-2xl shadow-lg uppercase tracking-tight"
        >
      ...
      <section className="py-20 px-6 bg-slate-950">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {[
            ...
            <div key={i} className="bg-slate-900 p-8 rounded-2xl border border-slate-800 hover:border-[#D4AF37] transition-all">
      ...
      <section className="py-20 bg-slate-950">
        ...
        <p className="text-slate-400 text-center mb-12">Veja como a rotina de estudos dessas famílias foi transformada.</p>
        
        <div className="relative max-w-lg mx-auto px-12">
          <div className="overflow-hidden" ref={emblaRef}>
            <div className="flex gap-4">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="flex-[0_0_100%] aspect-square bg-slate-800 rounded-2xl" />
              ))}
            </div>
          </div>
          <button onClick={() => emblaApi?.scrollPrev()} className="absolute left-0 top-1/2 -translate-y-1/2 p-2 bg-slate-800 rounded-full"><ChevronLeft /></button>
          <button onClick={() => emblaApi?.scrollNext()} className="absolute right-0 top-1/2 -translate-y-1/2 p-2 bg-slate-800 rounded-full"><ChevronRight /></button>
        </div>
      </section>
    </div>
  );
